...
import {POST_SESSIONS_429} from "../data/postSessions/postSessions429";
import {POST_SESSIONS_500} from "../data/postSessions/postSessions500";
...
import { GET_SESSIONS_500 } from "../data/getSessions/getSessions500";
...
export class YotiRequestProcessor {
    ...
    static getInstance(logger: Logger, metrics: Metrics, forceRetry: boolean = false): YotiRequestProcessor {
        if (
            !YotiRequestProcessor.instance ||
            YotiRequestProcessor.instance.forceRetry !== forceRetry
        ) {
    ...
    async createSession(event: APIGatewayProxyEvent, incomingPayload: any): Promise<Response> {
        ...
        // Retry on first call simulations
        switch (lastFullNameChars) {
            case '1429': // first attempt 429, second attempt 201
                this.logger.info({ message: "createSession – simulated 429 via FORCE_RETRY", lastFullNameChars });
                if (this.forceRetry) {
                return new Response(HttpCodesEnum.TOO_MANY_REQUESTS, JSON.stringify(POST_SESSIONS_429), ERROR_RESPONSE_HEADERS);
                }
                this.logger.warn({ message: "createSession – retry after 429, sleeping 5000 ms", retryCount: 0, yotiErrorStatus: 429, xRequestId: "dummy-request-id" });
                await sleep(5000);
                return new Response(HttpCodesEnum.CREATED, JSON.stringify(yotiSessionItem));

            case '1503': // first attempt 503, second attempt 201
                this.logger.info({ message: "createSession – simulated 503 via FORCE_RETRY", lastFullNameChars });
                if (this.forceRetry) {
                return new Response(HttpCodesEnum.SERVICE_UNAVAILABLE, JSON.stringify(POST_SESSIONS_503), ERROR_RESPONSE_HEADERS);
                }
                this.logger.warn({ message: "createSession – retry after 503, sleeping 5000 ms", retryCount: 0, yotiErrorStatus: 503, xRequestId: "dummy-request-id" });
                await sleep(5000);
                return new Response(HttpCodesEnum.CREATED, JSON.stringify(yotiSessionItem));

            case '1500': // first attempt 500, second attempt 201
                this.logger.info({ message: "createSession – simulated 500 via FORCE_RETRY", lastFullNameChars });
                if (this.forceRetry) {
                return new Response(HttpCodesEnum.SERVER_ERROR, JSON.stringify(POST_SESSIONS_500), ERROR_RESPONSE_HEADERS);
                }
                this.logger.warn({ message: "createSession – retry after 500, sleeping 5000 ms", retryCount: 0, yotiErrorStatus: 500, xRequestId: "dummy-request-id" });
                await sleep(5000);
                return new Response(HttpCodesEnum.CREATED, JSON.stringify(yotiSessionItem));
        }
    ...
    async getSession(sessionId: string): Promise<Response> {
        ...
        const processPositiveScenario = (lastUuidChars: string, sessionId: string): Response | undefined => {
            ...
            if (firstTwoChars === DocumentMapping.EEA_ID && this.forceRetry) {
                switch (lastUuidChars) {
                    case '0500': // simulate 500 on first call, CRI should retry
                    this.logger.info({ message: 'EEA guard: simulated 500 on first attempt', lastUuidChars });
                    return new Response(
                        HttpCodesEnum.SERVER_ERROR,
                        JSON.stringify(GET_SESSIONS_500),
                        ERROR_RESPONSE_HEADERS
                    );

                    case '0503': // simulate 503 on first call, CRI should retry
                    this.logger.info({ message: 'EEA guard: simulated 503 on first attempt', lastUuidChars });
                    return new Response(
                        HttpCodesEnum.SERVICE_UNAVAILABLE,
                        JSON.stringify(GET_SESSIONS_503),
                        ERROR_RESPONSE_HEADERS
                    );
                }
            }
        ...
        switch (lastUuidChars) {
            ...
            case '0503': // first attempt 503, second attempt 200 (EEA manual pass)
                this.logger.debug(JSON.stringify(yotiSessionRequest));
                if (this.forceRetry) {
                    this.logger.info({ message: "Simulated 503 on first attempt (constructor flag)", lastUuidChars });
                ...
                EEA_MANUAL_PASS_0503.resources.id_documents[0].document_fields.media.id = sessionId;
                EEA_MANUAL_PASS_0503.resources.id_documents[0].document_fields.media.id = replaceLastUuidChars(EEA_MANUAL_PASS_0503.resources.id_documents[0].document_fields.media.id, EEA_ID_MEDIA_ID);
                this.logger.warn({message: `fetchSessionInfo - Retrying after simulated 503. Sleeping for 5000 ms`, retryCount: 0, yotiErrorMessage: "Failed to fetch session info", yotiErrorCode: 503, yotiErrorStatus: 503, messageCode: "FAILED_YOTI_GET_SESSION", xRequestId: "dummy-request-id"});
                await sleep(5000);
                return new Response(HttpCodesEnum.OK, JSON.stringify(EEA_MANUAL_PASS_0503));

            case '1503': // createSession: first attempt 503, second attempt 201
                this.logger.debug(JSON.stringify(yotiSessionRequest));
                if (this.forceRetry) {
                    this.logger.warn({message: "createSession – Simulated 503 (constructor flag)", retryCount: 0, yotiErrorMessage: "Failed to create session", yotiErrorCode: 503, yotiErrorStatus: 503, messageCode: "FAILED_CREATING_YOTI_SESSION", xRequestId: "dummy-request-id"});
                    return new Response(HttpCodesEnum.SERVICE_UNAVAILABLE, JSON.stringify(POST_SESSIONS_503), ERROR_RESPONSE_HEADERS);
                }
                this.logger.warn({message: `createSession – Retrying to create Yoti session. Sleeping for 5000 ms`, retryCount: 0, yotiErrorMessage: "Failed to create session", yotiErrorCode: 503, yotiErrorStatus: 503, messageCode: "FAILED_CREATING_YOTI_SESSION", xRequestId: "dummy-request-id"});
                await sleep(5000);
                return new Response(HttpCodesEnum.CREATED, JSON.stringify(YotiSessionItem));
        }
