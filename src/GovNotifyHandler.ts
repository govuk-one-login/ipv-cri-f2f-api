...
const POWERTOOLS_METRICS_NAMESPACE = process.env.POWERTOOLS_METRICS_NAMESPACE
	...
	: Constants.EMAIL_METRICS_NAMESPACE;
...
const POWERTOOLS_SERVICE_NAME = process.env.POWERTOOLS_SERVICE_NAME
	...
	: Constants.EMAIL_LOGGER_SVC_NAME;
  	...
  	if (event.Records.length === 1) {
  		...
  		try {
  			...
  			if (!YOTI_PRIVATE_KEY) {
  				...
  				try {
  					YOTI_PRIVATE_KEY = await getParameter(
  						this.environmentVariables.yotiKeySsmPath(),
  					);
  				} catch (error) {
  					logger.error(
  						`failed to get param from ssm at ${this.environmentVariables.yotiKeySsmPath()}`,
  						{
  			...
  			if (!GOVUKNOTIFY_API_KEY) {
  				...
  				try {
  					GOVUKNOTIFY_API_KEY = await getParameter(
  						this.environmentVariables.govNotifyApiKeySsmPath(),
  					);
  				} catch (error) {
  					logger.error(
  						`failed to get param from ssm at ${this.environmentVariables.govNotifyApiKeySsmPath()}`,
  						{
  							messageCode: MessageCodes.MISSING_CONFIGURATION,
  							error,
  						},
  			...
  			try {
  				govnotifyServiceId = GOVUKNOTIFY_API_KEY.substring(
  					GOVUKNOTIFY_API_KEY.length - 73,
  					GOVUKNOTIFY_API_KEY.length - 37,
  				);
  			} catch (error) {
  				logger.error(
  					"failed to extract govnotifyServiceId from the GOVUKNOTIFY_API_KEY",
  					{ error },
  				);
  				return failEntireBatch;
  			}
  			await SendEmailProcessor.getInstance(
  				logger,
  				metrics,
  				YOTI_PRIVATE_KEY,
  				GOVUKNOTIFY_API_KEY,
  			...
  			logger.error({
  				message: "Email could not be sent. Returning failed message",
  				error,
  			});
  			return failEntireBatch;
  		}
