...
const POWERTOOLS_SERVICE_NAME = process.env.POWERTOOLS_SERVICE_NAME
	...
	: Constants.DOCUMENT_SELECTION_LOGGER_SVC_NAME;
  	...
  	try {
  		...
  		if (!YOTI_PRIVATE_KEY) {
  			...
  			try {
  				...
  				logger.error(
  					`failed to get param from ssm at ${this.YOTI_KEY_SSM_PATH}`,
  					{
  		...
  		return await DocumentSelectionRequestProcessor.getInstance(
  			logger,
  			metrics,
  			YOTI_PRIVATE_KEY,
