...
function convertRequestBodyObjectToString(body: { [key: string]: string }): string {
	...
	logger.info("-----bosyStr", formDataString);
...
accessTokenRouter.post("/", async (req, res) => {		
	logger.debug("AccessToken Request body: ", JSON.stringify(req.body) );
	...
	logger.info("AccessToken response: ", { response });
