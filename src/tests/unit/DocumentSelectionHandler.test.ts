...
const mockDocumentSelectionRequestProcessor =
  mock<DocumentSelectionRequestProcessor>();
jest.mock("../../utils/Config", () => {
	return {
		getParameter: jest.fn(() => {
			return "YOTIPRIVATEKEY";
		}),
	};
});
...
describe("DocumentSelectionHandler", () => {
	it("return Unauthorized if session id is missing", async () => {
		DocumentSelectionRequestProcessor.getInstance = jest
			.fn()
			.mockReturnValue(mockDocumentSelectionRequestProcessor);
