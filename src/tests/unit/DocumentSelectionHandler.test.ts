...
describe("DocumentSelectionHandler", () => {
	it("return not found when resource not found", async () => {
		...
		return expect(lambdaHandler(RESOURCE_NOT_FOUND, "")).resolves.toEqual(new Response(HttpCodesEnum.NOT_FOUND, `Requested resource does not exist ${RESOURCE_NOT_FOUND.resource}`));
	});
