...
import {
	INVALID_YOTI_TOPIC_SQS_EVENT,
	VALID_SESSION_COMPLETION_SQS_EVENT,
	VALID_THANK_YOU_SQS_EVENT,
} from "./data/sqs-events";
import {
	handlerClass,
	lambdaHandler,
	logger,
} from "../../TriggerYotiCallbackStateMachineHandler";
...
describe("TriggerYotiCallbackStateMachineHandler", () => {
	it("fails batch if incorrect number of records is passed", async () => {
		...
		expect(logger.warn).toHaveBeenCalledWith(
			"Unexpected no of records received",
			{
	...
	it("logs warning and passes batch if unrecognised topic is passed", async () => {
		...
		expect(logger.warn).toHaveBeenCalledWith(
			"Unexpected topic received in request",
			{
				topic: "unknown_event",
	...
	it.each([
		...
		"invokes step function when $topic event is passed",
		async ({ topic, event }) => {
			...
			expect(logger.info).toHaveBeenCalledWith(
				"Matched topic, triggering state machine",
				{ topic },
			);
	...
	it("error is returned if step function fails", async () => {
		...
		jest
			.spyOn(handlerClass.stepFunctionsClient, "send")
			.mockImplementationOnce(() => {
				throw error;
			});

		await expect(
			lambdaHandler(VALID_SESSION_COMPLETION_SQS_EVENT, "F2F"),
		).rejects.toThrow();
		expect(logger.error).toHaveBeenCalledWith({
			message: "There was an error executing the yoti callback step function",
			error,
		});
	});
});
