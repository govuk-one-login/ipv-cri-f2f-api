...
describe("ExpiredSessionsProcessor", () => {
	...
	it("should send message to IPVCore for expired sessions", async () => {
		...
		expect(mockF2fService.markSessionAsExpired).toHaveBeenNthCalledWith(1, "1b655a2e-44e4-4b21-a626-7825abd9c93e");
		expect(mockF2fService.markSessionAsExpired).toHaveBeenNthCalledWith(2, "3b655a2e-44e4-4b21-a626-7825abd9c93g");
		expect(mockF2fService.markSessionAsExpired).toHaveBeenNthCalledWith(3, "4b655a2e-44e4-4b21-a626-7825abd9c93h");
	...
	it("should log an error if not able to set the expired notification flag", async () => {
		mockF2fService.markSessionAsExpired.mockRejectedValueOnce(new Error("Unable to set expired notification flag"));
		mockF2fService.getSessionsByAuthSessionStates.mockResolvedValue(F2FSessionsWithYotiSession);
	...
	it("should throw an error if not able to access session table", async () => {
		mockF2fService.getSessionsByAuthSessionStates.mockRejectedValueOnce(new Error("Permission Denied"));
