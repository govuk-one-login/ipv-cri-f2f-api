...
import {
	personIdentityInputRecord,
	personIdentityOutputRecord,
} from "../data/personIdentity-records";
...
describe("F2f Service", () => {
	...
	it("Should return a session item when passed a valid session Id", async () => {
		mockDynamoDbClient.send = jest
			.fn()
			.mockResolvedValue({ Item: SESSION_RECORD });
	...
	it("Should not throw an error when session expiry date has passed", async () => {
		...
		mockDynamoDbClient.send = jest
			.fn()
			.mockResolvedValue({ Item: expiredSession });
		await expect(f2fService.getSessionById("1234")).rejects.toThrow(
	...
	it("should throw 500 if request fails when setting AuthorizationCode", async () => {
		...
		await expect(
			f2fService.setAuthorizationCode(FAILURE_VALUE, randomUUID()),
		).rejects.toThrow(
			expect.objectContaining({
	...
	it("should throw 500 if request fails during update Session data with access token details", async () => {
		...
		await expect(
			f2fService.updateSessionWithAccessTokenDetails(sessionId, 12345),
		).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
			}),
	...
	it("should throw 500 if request fails during update Session data with yoti session details", async () => {
		...
		await expect(
			f2fService.updateSessionWithYotiIdAndStatus(sessionId, "12345", "4567"),
		).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
			}),
	...
	it("Should log success if session details update with Yoti SessionId", async () => {
		...
		await f2fService.updateSessionWithYotiIdAndStatus(
			"123",
			"456",
			"YOTI_SESSION_CREATED",
		);
		expect(logger.info).toHaveBeenCalledWith({
			message: "Updated Yoti session details in dynamodb",
		});
	});
	...
	it("show throw error if failed to send to TXMA queue", async () => {
		...
		await expect(f2fService.sendToTXMA(txmaEventPayload)).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
			}),
		);
	...
	it("show throw error if failed to send to GovNotify queue", async () => {
		...
		await expect(
			f2fService.sendToGovNotify(govNotifyEventPayload),
		).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
			}),
	...
	it("should return undefined when session item is not found by authorization code", async () => {
		...
		await expect(
			f2fService.getSessionByAuthorizationCode("1234"),
		).rejects.toThrow("Error retrieving Session by authorization code");
	});

	it("should throw error when multiple session items are found by authorization code", async () => {
		mockDynamoDbClient.query = jest
			.fn()
			.mockResolvedValue({
				Items: [{ sessionId: "SESSID1" }, { sessionId: "SESSID2" }],
			});
	...
	it("should return empty array when session items are not found by auth session state", async () => {
		...
		const result = await f2fService.getSessionsByAuthSessionStates([
			"F2F_SESSION_STARTED",
		]);
					...
					input: expect.objectContaining({
						Item: expect.objectContaining({
							expiryDate: Math.floor(
								fakeTime + +process.env.AUTH_SESSION_TTL_SECS!,
							),
	...
	it("Should throw an error when session by yoti sessionId doesn't exist", async () => {
		mockDynamoDbClient.send = jest.fn().mockResolvedValue({});
		await expect(f2fService.getSessionByYotiId("1234")).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
		...
		expect(logger.error).toHaveBeenCalledWith(
			{ message: "Error retrieving Session by yoti session id" },
			{ messageCode: "FAILED_FETCHING_BY_YOTI_SESSIONID" },
		);
	});

	it("Should throw an error when Person record by sessionId doesn't exist", async () => {
		mockDynamoDbClient.send = jest.fn().mockRejectedValue({});
		await expect(f2fService.getPersonIdentityById("1234")).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
		...
		expect(logger.error).toHaveBeenCalledWith(
			...
			{ error: {}, messageCode: "FAILED_FETCHING_PERSON_IDENTITY" },
		);
	});
	...
	it.each([
		...
		[
			"should update person identity table with updated ttl",
			123456,
			"PERSONTABLE",
		],
	...
	it.each([
		...
		mockDynamoDbClient.send = jest.fn().mockRejectedValue({});
		await expect(
			f2fService.updateSessionTtl(FAILURE_VALUE, 123456, testTableName),
		).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
	...
	it("should update session table with documentUsed", async () => {
		mockDynamoDbClient.send = jest.fn().mockResolvedValue({});
		await f2fService.addUsersSelectedDocument(
			sessionId,
			"passport",
			"SESSIONTABLE",
		);
	...
	it("should throw 500 if fails to update documentUsed", async () => {
		mockDynamoDbClient.send = jest.fn().mockRejectedValue({});
		await expect(
			f2fService.addUsersSelectedDocument(sessionId, "passport", "SESSIONTABLE"),
		).rejects.toThrow(
			expect.objectContaining({
				statusCode: HttpCodesEnum.SERVER_ERROR,
	...
	describe("obfuscateJSONValues", () => {
		it("should obfuscate all fields except those in txmaFieldsToShow", async () => {
			...
			const obfuscatedObject = await f2fService.obfuscateJSONValues(
				inputObject,
				txmaFieldsToShow,
			);
