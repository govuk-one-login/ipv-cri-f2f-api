...
import {
  TXMA_CORE_FIELDS,
  TXMA_DL_VC_ISSUED,
  TXMA_DL_VC_ISSUED_WITHOUT_FULL_ADDRESS,
  TXMA_EEA_VC_ISSUED,
  TXMA_EU_DL_VC_ISSUED,
  TXMA_VC_ISSUED,
} from "../data/txmaEvent";
import {
  getCompletedYotiSession,
  getDocumentFields,
  getDrivingPermitFields,
  getDrivingPermitFieldsWithoutFormattedAddress,
  getEeaIdCardFields,
  getEuDrivingPermitFields,
} from "../utils/YotiCallbackUtils";
...
const mockGenerateVerifiableCredential = mock<GenerateVerifiableCredential>();
...
describe("YotiSessionCompletionProcessor", () => {
  let f2fSessionItem: ISessionItem,
    ...
    completedYotiSession: YotiCompletedSession;

  beforeEach(() => {
    // Reset mocks, timers, and states before each test
    jest.resetAllMocks(); // Keeps the mock calls reset but not the implementations
    jest.useFakeTimers();
    jest.setSystemTime(new Date(1585695600000));

    mockCompletedSessionProcessor = new YotiSessionCompletionProcessor(
      logger,
      metrics,
      "YOTIPRIM",
    );
  ...
  it("Return successful response with 200 OK when YOTI session created with UK Passport", async () => {
    ...
    mockYotiService.getCompletedSessionInfo.mockResolvedValueOnce(
      completedYotiSession,
    );
    ...
    // @ts-expect-error linting to be updated
    mockCompletedSessionProcessor.verifiableCredentialService.kmsJwtAdapter =
      passingKmsJwtAdapterFactory();

    const out: APIGatewayProxyResult =
    ...
    expect(metrics.addMetric).toHaveBeenNthCalledWith(
      1,
      "SessionCompletion_yoti_response_parsed",
      MetricUnits.Count,
      1,
    );
    expect(metrics.addMetric).toHaveBeenNthCalledWith(
      2,
      "state-F2F_CREDENTIAL_ISSUED",
      MetricUnits.Count,
      1,
    );
    expect(metrics.addMetric).toHaveBeenNthCalledWith(
      3,
      "SessionCompletion_VC_issued_successfully",
      MetricUnits.Count,
      1,
    );
  });
  ...
  it("Return successful response with 200 OK when YOTI session created with driving permit", async () => {
    const documentFields = getDrivingPermitFields();
  ...
  it("Return successful response with 200 OK when YOTI session is completed for driving permit- Media not containing formatted_address", async () => {
    const documentFields = getDrivingPermitFieldsWithoutFormattedAddress();
    const ukDLYotiSession = getCompletedYotiSession();
    ukDLYotiSession.resources.id_documents[0].document_type = "DRIVING_LICENCE";
    mockYotiService.getCompletedSessionInfo.mockResolvedValueOnce(
      ukDLYotiSession,
  ...
  it("Return successful response with 200 OK when YOTI session created with EU driving permit", async () => {
    const documentFields = getEuDrivingPermitFields();
  ...
  it("Return successful response with 200 OK when YOTI session created with EEA Identity Card", async () => {
    const documentFields = getEeaIdCardFields();
  ...
  describe("name checks", () => {
    it("Should call getNamesFromYoti if DocumentFields contains both given_name and family_name fields", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "Getting NameParts using Yoti DocumentFields Info",
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_yoti_response_parsed",
    ...
    it("Should call getNamesFromPersonIdentity if DocumentFields does not contain given_name", async () => {
      ...
      mockF2fService.getPersonIdentityById.mockResolvedValueOnce(
        personIdentityItem,
      );
      mockF2fService.getSessionByYotiId.mockResolvedValueOnce(f2fSessionItem);
      // @ts-expect-error linting to be updated
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "Getting NameParts using F2F Person Identity Info",
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_yoti_response_parsed",
    ...
    it("Should throw an error of name mismatch between F2F and Yoti DocumentFields", async () => {
      ...
      expect(
        mockCompletedSessionProcessor.processRequest(VALID_REQUEST),
      ).rejects.toThrow(
  ...
  describe("yoti session info", () => {
    it("Throw server error if completed Yoti session can not be found", async () => {
      ...
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Yoti Session not found",
        {
      ...
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_Yoti_session_not_found",
        MetricUnits.Count,
        1,
      );
      expect(mockF2fService.sendToIPVCore).toHaveBeenCalledWith({
    ...
    it("Throws server error if session in Yoti is not completed", async () => {
      const documentFields = getDocumentFields();
      const completedYotiSessionClone = {
        ...completedYotiSession,
        state: "ONGOING",
      };
      mockYotiService.getCompletedSessionInfo.mockResolvedValueOnce(
        completedYotiSessionClone,
      );
      mockYotiService.getMediaContent.mockResolvedValueOnce(documentFields);
      mockF2fService.getSessionByYotiId.mockResolvedValueOnce(f2fSessionItem);
      ...
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Yoti Session not complete",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_Yoti_session_not_complete",
        MetricUnits.Count,
        1,
      );
      expect(mockF2fService.sendToIPVCore).toHaveBeenCalledWith({
    ...
    it("Throws server error if session in Yoti does not contain document fields", async () => {
      const documentFields = getDocumentFields();
      const completedYotiSessionClone = JSON.parse(
        JSON.stringify(completedYotiSession),
      );
      delete completedYotiSessionClone.resources.id_documents[0]
        .document_fields;
      ...
      expect(logger.error).toHaveBeenCalledWith(
        { message: "No document_fields found in completed Yoti Session" },
        {
      ...
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_Yoti_document_fields_not_populated",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Yoti document_fields not populated",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
    ...
    it("Throws server error if session in Yoti contains multiple document_field entries", async () => {
      ...
      completedYotiSessionClone.resources.id_documents = [
        completedYotiSession.resources.id_documents[0],
        completedYotiSession.resources.id_documents[0],
      ];
      ...
      expect(logger.error).toHaveBeenCalledWith(
        { message: "Multiple document_fields found in completed Yoti Session" },
        {
      ...
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_multiple_document_fields_in_response",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Multiple document_fields in response",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
    ...
    it("Throws server error if session in Yoti does not contain media ID", async () => {
      ...
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_Yoti_document_fields_media_ID_not_found",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Yoti document_fields media ID not found",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
    ...
    it("Throws server error if session in Yoti does not contain document fields info", async () => {
      const completedYotiSessionClone = JSON.parse(
        JSON.stringify(completedYotiSession),
      );
      mockYotiService.getCompletedSessionInfo.mockResolvedValueOnce(
      ...
      await expect(
        ...
        expect.objectContaining({
          statusCode: HttpCodesEnum.SERVER_ERROR,
          message: "Yoti document fields info not found",
        }),
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        1,
        "SessionCompletion_Yoti_document_fields_info_not_found",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Yoti document fields info not found",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
    ...
    it("Throws server error if session in Yoti is missing first, family and full name", async () => {
      const documentFields = getDocumentFields();
      const completedYotiSessionClone = JSON.parse(
        JSON.stringify(completedYotiSession),
      );
      ...
      delete documentFields.full_name;
      delete documentFields.family_name;
      delete documentFields.given_names;
      console.log("localDocumentFields", documentFields);
      mockYotiService.getMediaContent.mockResolvedValueOnce(documentFields);
      mockF2fService.getPersonIdentityById.mockResolvedValueOnce(
        personIdentityItem,
      );
      ...
      await expect(
        ...
        expect.objectContaining({
          statusCode: HttpCodesEnum.SERVER_ERROR,
          message: "Missing Name Info in DocumentFields",
        }),
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        2,
        "SessionCompletion_Missing_Name_Info_in_DocumentFields",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
        2,
        "VC generation failed : Missing Name Info in DocumentFields",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
    ...
    it("Throws bad request error if details missing in person identity table", async () => {
      const documentFields = getDocumentFields();
      delete documentFields.family_name;
      delete documentFields.given_names;
      const completedYotiSessionClone = JSON.parse(
      ...
      await expect(
        ...
        expect.objectContaining({
          ...
          message: "Missing details in PERSON IDENTITY tables",
        }),
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        2,
        "SessionCompletion_Missing_details_in_PERSON_IDENTITY_tables",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
        1,
        "VC generation failed : Missing details in PERSON IDENTITY tables",
        {
          messageCode: MessageCodes.ERROR_GENERATING_VC,
        },
      );
    ...
    it("Throws server error if credential subject or evidence payload are missing", async () => {
      const documentFields = getDocumentFields();
      mockF2fService.getSessionByYotiId.mockResolvedValueOnce(f2fSessionItem);
      mockYotiService.getCompletedSessionInfo.mockResolvedValueOnce(
        completedYotiSession,
      ...
      // @ts-expect-error linting to be updated
      mockCompletedSessionProcessor.generateVerifiableCredential =
        mockGenerateVerifiableCredential;
      (
        mockGenerateVerifiableCredential.getVerifiedCredentialInformation as jest.Mock
      ...
      await expect(
        ...
        expect.objectContaining({
          statusCode: HttpCodesEnum.SERVER_ERROR,
          message: "Missing Credential Subject or Evidence payload",
        }),
      );
      expect(metrics.addMetric).toHaveBeenNthCalledWith(
        2,
        "SessionCompletion_Missing_Credential_Subject_or_Evidence_payload",
        MetricUnits.Count,
        1,
      );
      expect(logger.error).toHaveBeenNthCalledWith(
      ...
      expect(mockF2fService.sendToIPVCore).toHaveBeenCalledWith({
        ...
        error: "access_denied",
        error_description:
          "VC generation failed : Missing Credential Subject or Evidence payload",
      });
    });
  });
  ...
  it("Should throw an error when session is in wrong AuthSessionState", async () => {
    ...
    expect(logger.error).toHaveBeenNthCalledWith(
      2,
      "VC generation failed : AuthSession is in wrong Auth state",
      {
        messageCode: MessageCodes.ERROR_GENERATING_VC,
      },
    );
    expect(metrics.addMetric).toHaveBeenNthCalledWith(
      1,
      "SessionCompletion_AuthSession_in_wrong_Auth_state",
      MetricUnits.Count,
      1,
    );
    expect(mockF2fService.sendToIPVCore).toHaveBeenCalledWith({
    ...
    expect(out.body).toBe(
      "AuthSession is in wrong Auth state: Expected state- F2F_ACCESS_TOKEN_ISSUED or F2F_AUTH_CODE_ISSUED, actual state- F2F_YOTI_SESSION_CREATED",
    );
  });
  ...
  it("Return 200 when write to txMA fails", async () => {
    ...
    expect(logger.error).toHaveBeenNthCalledWith(
      1,
      "Failed to write TXMA event F2F_YOTI_RESPONSE_RECEIVED to SQS queue.",
      { messageCode: MessageCodes.FAILED_TO_WRITE_TXMA },
    );
    expect(logger.error).toHaveBeenNthCalledWith(
      2,
      "Failed to write TXMA event F2F_CRI_VC_ISSUED to SQS queue.",
      { error: {}, messageCode: MessageCodes.FAILED_TO_WRITE_TXMA },
    );
    expect(out.statusCode).toBe(HttpCodesEnum.OK);
  });
  ...
  it("Throws server error if failure to send to IPVCore queue", async () => {
    ...
    mockF2fService.sendToIPVCore.mockRejectedValueOnce(
      "Failed to send to IPV Core",
    );
  ...
  it("Throws server error if signGeneratedVerifiableCredentialJwt returns empty string", async () => {
    ...
    jest
      .spyOn(
        VerifiableCredentialService.prototype as any,
        "signGeneratedVerifiableCredentialJwt",
      )
      .mockReturnValue("");
    ...
    expect(logger.error).toHaveBeenNthCalledWith(
      2,
      "VC generation failed : Unable to create signed JWT",
      {
        messageCode: MessageCodes.ERROR_GENERATING_VC,
      },
    );
    ...
    expect(metrics.addMetric).toHaveBeenNthCalledWith(
      2,
      "SessionCompletion_Unable_to_create_signed_JWT",
      MetricUnits.Count,
      1,
    );
    expect(metrics.addMetric).not.toHaveBeenNthCalledWith(
      3,
      "SessionCompletion_VC_issued_successfully",
      MetricUnits.Count,
      1,
  ...
  it("Returns server error response if signGeneratedVerifiableCredentialJwt throws error", async () => {
    ...
    expect(logger.error).toHaveBeenNthCalledWith(
      2,
      "VC generation failed : Failed to sign the verifiableCredential Jwt",
      {
        messageCode: MessageCodes.ERROR_GENERATING_VC,
      },
    );
    expect(metrics.addMetric).toHaveBeenNthCalledWith(
      2,
      "SessionCompletion_Failed_to_sign_verifiableCredential_Jwt",
      MetricUnits.Count,
      1,
    );
    expect(mockF2fService.sendToIPVCore).toHaveBeenCalledWith({
  ...
  describe("isTaskDone function", () => {
    it('should return true if a task with the specified type and state "DONE" exists', () => {
      ...
      const result = mockCompletedSessionProcessor.isTaskDone(
        data,
        taskTypeToCheck,
      );
