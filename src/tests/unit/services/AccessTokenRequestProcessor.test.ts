...
import {
	MISSING_BODY_ACCESSTOKEN,
	VALID_ACCESSTOKEN,
} from "../data/accessToken-events";
...
describe("AccessTokenRequestProcessor", () => {
	...
	beforeEach(() => {
		jest.clearAllMocks();
		// @ts-ignore
		accessTokenRequestProcessorTest.kmsJwtAdapter =
	...
	it("Return bearer access token response when grant_type, code, and redirect_uri parameters are provided", async () => {
		...
		const out: Response = await accessTokenRequestProcessorTest.processRequest(
			request,
		);
		// eslint-disable-next-line @typescript-eslint/unbound-method
		expect(mockF2fService.getSessionByAuthorizationCode).toHaveBeenCalledTimes(
			1,
		);
	...
	it.each([
		[
			`grant_type=authorization_code&redirect_uri=${ENCODED_REDIRECT_URI}`,
			"Invalid request: Missing code parameter",
		],
		[
			`code=${AUTHORIZATION_CODE}&redirect_uri=${ENCODED_REDIRECT_URI}`,
		...
		"When parameters are not provided in the body, it returns 401 Unauthorized response",
		async (body, errMsg) => {
			request.body = body;
			const out: Response =
        await accessTokenRequestProcessorTest.processRequest(request);
	...
	it("Return 401 Unauthorized response when AuthSessionState is not F2F_AUTH_CODE_ISSUED", async () => {
		...
		expect(out.body).toBe(
			"Session is in the wrong state: F2F_ACCESS_TOKEN_ISSUED",
		);
		expect(out.statusCode).toBe(HttpCodesEnum.UNAUTHORIZED);
	});
	...
	it("Returns 401 Unauthorized response when redirect_uri parameter does not match the value in SessionTable", async () => {
		...
		expect(out.body).toBe(
			"Invalid request: redirect uri TEST_REDIRECT_URI does not match configuration uri http://localhost:8085/callback",
		);
		expect(out.statusCode).toBe(HttpCodesEnum.UNAUTHORIZED);
	});
	...
	it("Return 401 Unauthorized response when session was not found in the DB for a authorizationCode", async () => {
		...
		expect(out.body).toBe(
			"No session found by authorization code: " + AUTHORIZATION_CODE,
		);
		expect(out.statusCode).toBe(HttpCodesEnum.UNAUTHORIZED);
	});
	...
	it("Return 500 Server Error when Failed to sign the access token Jwt", async () => {
		// @ts-ignore
		accessTokenRequestProcessorTest.kmsJwtAdapter =
      failingKmsJwtSigningAdapterFactory();
		const out: Response = await accessTokenRequestProcessorTest.processRequest(
			request,
		);
