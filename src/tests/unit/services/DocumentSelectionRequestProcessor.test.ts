...
describe("DocumentSelectionRequestProcessor", () => {
	...
	it("Throw server error if Yoti Session already exists", async () => {
		...
		expect(out.body).toBe("Yoti session already exists or session for journey sdfssg is in the wrong state: F2F_YOTI_SESSION_CREATED, expected state should be F2F_SESSION_CREATED");
		expect(logger.warn).toHaveBeenCalledWith(
			"Yoti session already exists or session for journey sdfssg is in the wrong state: F2F_YOTI_SESSION_CREATED, expected state should be F2F_SESSION_CREATED", { messageCode: "INCORRECT_SESSION_STATE" },
		);
