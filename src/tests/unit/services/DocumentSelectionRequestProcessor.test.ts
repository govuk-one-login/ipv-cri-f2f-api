...
import {
	TXMA_NATIONAL_ID_YOTI_START,
	TXMA_PASSPORT_YOTI_START,
} from "../data/txmaEvent";
...
describe("DocumentSelectionRequestProcessor", () => {
	let personIdentityItem: PersonIdentityItem,
		f2fSessionItem: ISessionItem,
		yotiSessionInfo: YotiSessionInfo;
	beforeAll(() => {
		mockDocumentSelectionRequestProcessor =
      new DocumentSelectionRequestProcessor(logger, metrics, "YOTIPRIM");
		// @ts-ignore
	...
	it("Return successful response with 200 OK when YOTI session created", async () => {
		...
		mockYotiService.createSession.mockResolvedValueOnce(
			"b83d54ce-1565-42ee-987a-97a1f48f27dg",
		);

		mockYotiService.fetchSessionInfo.mockResolvedValueOnce(yotiSessionInfo);

		mockYotiService.generateInstructions.mockResolvedValueOnce(
			HttpCodesEnum.OK,
		);
		...
		const out: Response =
      await mockDocumentSelectionRequestProcessor.processRequest(
      	VALID_REQUEST,
      	"RandomF2FSessionID",
      );
		...
		// eslint-disable-next-line @typescript-eslint/unbound-method
		expect(
			mockF2fService.updateSessionWithYotiIdAndStatus,
	...
	it("Should return successful response with 200 OK when non-UK passport used for YOTI session", async () => {
		yotiSessionInfo.capture.required_resources[0].supported_countries[0].code =
      "ESP";
	...
	it("Should return successful response with 200 OK when an EEA ID Card is used and creates YOTI session", async () => {
		yotiSessionInfo.capture.required_resources[0].supported_countries[0].code =
      "IRL";
		yotiSessionInfo.capture.required_resources[0].supported_countries[0].supported_documents[0].type =
      "NATIONAL_ID";
	...
	it("Throw bad request error when personDetails is missing", async () => {
		...
		await expect(
			mockDocumentSelectionRequestProcessor.processRequest(
				VALID_REQUEST,
				"1234",
			),
		).rejects.toThrow(
		...
		expect(logger.warn).toHaveBeenNthCalledWith(
			1,
			"Missing details in SESSION or PERSON IDENTITY tables",
	...
	it("Should update the TTL on both Session & Person Identity Tables", async () => {
		...
		expect(mockF2fService.updateSessionTtl).toHaveBeenNthCalledWith(
			1,
			"RandomF2FSessionID",
			Math.floor(fakeTime + +process.env.AUTH_SESSION_TTL_SECS!),
			"SESSIONTABLE",
		);
		expect(mockF2fService.updateSessionTtl).toHaveBeenNthCalledWith(
			2,
	...
	it("Should save the documentUsed type in session table", async () => {
		...
		expect(mockF2fService.addUsersSelectedDocument).toHaveBeenCalledWith(
			"RandomF2FSessionID",
			"PASSPORT",
			"SESSIONTABLE",
		);
	...
	it("Throw server error if Yoti Session already exists", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(out.body).toBe(
			"Yoti session already exists for this authorization session or Session is in the wrong state",
		);
		expect(logger.warn).toHaveBeenCalledWith(
			"Yoti session already exists for this authorization session or Session is in the wrong state: F2F_YOTI_SESSION_CREATED",
	...
	it("Returns server error if PersonIdentity table is missing emailAddress", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(logger.error).toHaveBeenCalledWith(
			"Missing emailAddress in the PERSON IDENTITY table",
	...
	it("Returns server error if PersonIdentity table is missing name", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(out.statusCode).toBe(HttpCodesEnum.SERVER_ERROR);
		expect(out.body).toBe(
			"Missing person's GivenName or FamilyName in the PERSON IDENTITY table",
		);
		expect(logger.error).toHaveBeenCalledWith(
			"Missing person's GivenName or FamilyName in the PERSON IDENTITY table",
	...
	it("Throw server error if Yoti Session creation fails", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(logger.error).toHaveBeenNthCalledWith(
			1,
			"An error occurred when creating Yoti Session",
			{ messageCode: "FAILED_CREATING_YOTI_SESSION" },
		);
		expect(logger.error).toHaveBeenNthCalledWith(
			2,
			"Error occurred during documentSelection orchestration",
			"An error occurred when creating Yoti Session",
			{ messageCode: "FAILED_DOCUMENT_SELECTION_ORCHESTRATION" },
		);
	});
	...
	it("Throw server error if Yoti Session info fetch fails", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(logger.error).toHaveBeenNthCalledWith(
			1,
			"An error occurred when fetching Yoti Session",
			{ messageCode: "FAILED_FETCHING_YOTI_SESSION" },
		);
		expect(logger.error).toHaveBeenNthCalledWith(
			2,
			"Error occurred during documentSelection orchestration",
	...
	it("Throw server error if Yoti pdf generation fails", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(out.statusCode).toBe(HttpCodesEnum.SERVER_ERROR);
		expect(out.body).toBe(
			"An error occurred when generating Yoti instructions pdf",
		);
		expect(logger.error).toHaveBeenCalledTimes(2);
		expect(logger.error).toHaveBeenNthCalledWith(
			1,
	...
	it("Return 200 when write to txMA fails", async () => {
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		// eslint-disable-next-line @typescript-eslint/unbound-method
		expect(logger.error).toHaveBeenCalledWith(
			"Failed to write TXMA event F2F_YOTI_START to SQS queue.",
	...
	it("Throws server error if failure to send to GovNotify queue", async () => {
		...
		mockF2fService.sendToGovNotify.mockRejectedValueOnce(
			"Failed to send to GovNotify Queue",
		);
      ...
      await mockDocumentSelectionRequestProcessor.processRequest(
		...
		expect(out.statusCode).toBe(HttpCodesEnum.SERVER_ERROR);
		expect(out.body).toBe(
			"An error occurred when sending message to GovNotify handler",
		);
		expect(logger.error).toHaveBeenCalledTimes(2);
		expect(logger.error).toHaveBeenNthCalledWith(
			1,
			"Yoti session created, failed to post message to GovNotify SQS Queue",
			{
				error: "Failed to send to GovNotify Queue",
				messageCode: "FAILED_TO_WRITE_GOV_NOTIFY",
			},
		);
		expect(logger.error).toHaveBeenNthCalledWith(
			2,
	...
	it("Return 500 when updating the session returns an error", async () => {
		...
		mockF2fService.updateSessionWithYotiIdAndStatus.mockRejectedValueOnce(
			"Got error saving Yoti session details",
		);
	...
	it("Return 500 when updating the TTLs returns an error", async () => {
		...
		mockF2fService.updateSessionTtl.mockRejectedValueOnce(
			"Got error updating SESSIONTABLE ttl",
		);
	...
	it("Return 500 when add users documentUsed returns an error", async () => {
		...
		mockF2fService.addUsersSelectedDocument.mockRejectedValueOnce(
			"Got error updating SESSIONTABLE ttl",
		);
