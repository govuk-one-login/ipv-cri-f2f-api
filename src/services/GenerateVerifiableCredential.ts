...
export class GenerateVerifiableCredential {
  ...
  private calculateStrengthScore(documentType: string, documentContainsValidChip: boolean): number {
  	switch (documentType) {
  		...
  		case YotiDocumentTypesEnum.UKPHOTOCARDDL:
  			return 3;
  			// case EEA National Identity Card:
  			//   return validChipCheck ? 4 : 3;
  		default:
  ...
  private getCounterIndicator(
  			...
  			switch (reason) {
  				...
  				default:
  					throw new AppError(
  						...
  						"Unexpected reason for rejection" + { reason },
  					);
  			}
  		}
  	};
  		...
  		if (value === "REJECT") {
  			switch (reason) {
  				...
  				case "CHIP_CSCA_VERIFICATION_FAILED":
  					addToCI("D14");
  					break;
  					// Handle other rejection cases as needed
  				default:
  					throw new AppError(
  						HttpCodesEnum.SERVER_ERROR,
  						"Unexpected reason for rejection" + { reason },
  	...
  	// Call helper functions to handle different recommendations
  ...
  getVerifiedCredentialInformation(this: any, 
  	...
  	const evidence: VerifiedCredentialEvidence = [
  		{
  			type: "IdentityCheck",
  			strengthScore: this.calculateStrengthScore(documentType, documentContainsValidChip),
  	...
  	if (
  		...
  		evidence[0].ci = this.getCounterIndicator(MANDATORY_CHECKS.ID_DOCUMENT_FACE_MATCH?.recommendation, MANDATORY_CHECKS.ID_DOCUMENT_AUTHENTICITY?.recommendation),
