...
import {
  EuDrivingLicenseCountry,
  EU_DL_COUNTRIES,
} from "../models/EuDrivingLicenceCodes";
import {
...
export class GenerateVerifiableCredential {
   ...
   */
  private doesDocumentContainValidChip(
    documentType: string,
    documentAuthenticityCheckBreakdown: Array<{
      sub_check: string;
      result: string;
    }>,
  ): boolean {
    if (DOCUMENT_TYPES_WITH_CHIPS.includes(documentType)) {
      const validChip = documentAuthenticityCheckBreakdown.some(
        (subCheck: { sub_check: string; result: string }) =>
          subCheck.sub_check === YotiSessionDocument.CHIP_CSCA_TRUSTED &&
      ...
      switch (documentType) {
        ...
        default:
          throw new AppError(
            ...
            "Invalid documentType provided for issuingCountry",
            {
              documentType,
              issuingCountry,
            },
   ...
   **/
  private calculateVerificationProcessLevel(
    validityScore: number,
    faceMatchCheck: string,
  ): number {
    return faceMatchCheck === YotiSessionDocument.APPROVE && validityScore !== 0
      ? 3
      : 0;
  }
      ...
      if (value === "REJECT") {
        switch (reason) {
          ...
          case "DIFFERENT_PERSON":
            ...
            this.logger.info({
              message: "Handling face match rejection",
              reason,
              contraIndicator: "V01",
            });
            break;
    ...
    credentialSubject.address = [
      {
        ...
        streetName: postalAddress?.address_line1
          ?.match(/[a-zA-Z\s]+/)?.[0]
          ?.trim(),
    ...
    if (issuingCountry === "GBR") {
      ...
      switch (documentType) {
        case "PASSPORT":
          ...
          break;
        case "DRIVING_LICENCE":
          countryDetails = EU_DL_COUNTRIES.find(
            (country) => country.alpha3code === documentFields.issuing_country,
          );
          if (!countryDetails) {
            throw new AppError(
              HttpCodesEnum.SERVER_ERROR,
              "Unable to fetch the alpha2code for the EU country",
              {
                documentFields,
              },
            );
  ...
  getVerifiedCredentialInformation(
    ...
    const docInfo = {
      ...
      issuingCountry: documentFields.issuing_country
        ? documentFields.issuing_country
        : yotiCountryCode,
      issueDate: documentFields.date_of_issue,
      expiryDate: documentFields.expiration_date,
    };
    ...
    const findCheck = (type: string) =>
      checks.find(
        (checkCompleted: { type: string }) => checkCompleted.type === type,
      );
    ...
    const getCheckObject = (check: any) => {
      ...
      this.logger.info("Checks result:", {
        Check: checkObject.object,
        State: checkObject.state,
        Recommendation: checkObject.recommendation,
        Breakdown: checkObject.breakdown,
      });
      return checkObject;
    };
    ...
    const MANDATORY_CHECKS = {
      ID_DOCUMENT_AUTHENTICITY: findCheck(
        YOTI_CHECKS.ID_DOCUMENT_AUTHENTICITY.type,
      )
        ? getCheckObject(findCheck(YOTI_CHECKS.ID_DOCUMENT_AUTHENTICITY.type))
        : null,
      ID_DOCUMENT_FACE_MATCH: findCheck(YOTI_CHECKS.ID_DOCUMENT_FACE_MATCH.type)
        ? getCheckObject(findCheck(YOTI_CHECKS.ID_DOCUMENT_FACE_MATCH.type))
        : null,
      IBV_VISUAL_REVIEW_CHECK: findCheck(
        YOTI_CHECKS.IBV_VISUAL_REVIEW_CHECK.type,
      )
        ? getCheckObject(findCheck(YOTI_CHECKS.IBV_VISUAL_REVIEW_CHECK.type))
        : null,
      DOCUMENT_SCHEME_VALIDITY: findCheck(
        YOTI_CHECKS.DOCUMENT_SCHEME_VALIDITY_CHECK.type,
      )
        ? getCheckObject(
            findCheck(YOTI_CHECKS.DOCUMENT_SCHEME_VALIDITY_CHECK.type),
          )
        : null,
      PROFILE_DOCUMENT_MATCH: findCheck(YOTI_CHECKS.PROFILE_DOCUMENT_MATCH.type)
        ? getCheckObject(findCheck(YOTI_CHECKS.PROFILE_DOCUMENT_MATCH.type))
        : null,
    };
    ...
    if (
      ...
      throw new AppError(
        HttpCodesEnum.BAD_REQUEST,
        "Mandatory checks not all completed",
      );
    }
    ...
    credentialSubject = this.attachPersonName(credentialSubject, VcNameParts);

    //Attach individuals DOB to the VC payload
    credentialSubject = this.attachDOB(
      credentialSubject,
    ...
    this.logger.info(
      { message: "Checking if document contains a valid chip" },
      { documentContainsValidChip },
    );

    const manualFaceMatchCheck =
      MANDATORY_CHECKS.ID_DOCUMENT_FACE_MATCH?.breakdown.some(
        (subCheck: { sub_check: string; result: string }) =>
    ...
    this.logger.info({
      message: "Result of Manual FaceMatch Check",
      manualFaceMatchCheck,
    });

    const validityScore = this.calculateValidityScore(
      MANDATORY_CHECKS.ID_DOCUMENT_AUTHENTICITY?.recommendation.value,
      documentContainsValidChip,
    );
    const verificationScore = this.calculateVerificationProcessLevel(
      validityScore,
    ...
    const DocumentAuthenticity =
      MANDATORY_CHECKS.ID_DOCUMENT_AUTHENTICITY?.recommendation;
    if (
      validityScore === 0 &&
      DocumentAuthenticity.value &&
      DocumentAuthenticity.reason
    ) {
      this.logger.info("Validity Score 0", {
        value: DocumentAuthenticity.value,
        reason: DocumentAuthenticity.reason,
      });
    }
    ...
    if (
      ...
      const { contraIndicators, rejectionReasons: collectedRejectionReasons } =
        this.getContraIndicator(
          MANDATORY_CHECKS.ID_DOCUMENT_FACE_MATCH?.recommendation,
          MANDATORY_CHECKS.ID_DOCUMENT_AUTHENTICITY?.recommendation,
        );
