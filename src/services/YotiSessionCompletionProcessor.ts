...
export class YotiSessionCompletionProcessor {
  ...
  constructor(logger: Logger, metrics: Metrics, YOTI_PRIVATE_KEY: string) {
    ...
    this.verifiableCredentialService = VerifiableCredentialService.getInstance(
      ...
      this.logger,
      this.environmentVariables.dnsSuffix(),
    );
    this.generateVerifiableCredential =
      GenerateVerifiableCredential.getInstance(this.logger);
  ...
  isTaskDone(data: any, taskType: string): boolean {
    if (data.tasks && Array.isArray(data.tasks)) {
      for (const task of data.tasks) {
        if (
          task.type === taskType &&
          task.state === YotiSessionDocument.DONE_STATE
        ) {
          return true;
        }
      }
  ...
  static getInstance(
    ...
    if (!YotiSessionCompletionProcessor.instance) {
      YotiSessionCompletionProcessor.instance =
        new YotiSessionCompletionProcessor(logger, metrics, YOTI_PRIVATE_KEY);
    }
  ...
  async processRequest(
    ...
    this.logger.info(
      { message: "Fetching F2F Session info with Yoti SessionID" },
      { yotiSessionID },
    );
    if (yotiSessionID) {
      const f2fSession =
        await this.f2fService.getSessionByYotiId(yotiSessionID);
      ...
      const completedYotiSessionInfo =
        await this.yotiService.getCompletedSessionInfo(
          yotiSessionID,
      ...
      if (!completedYotiSessionInfo) {
        ...
        await this.sendErrorMessageToIPVCore(
          f2fSession,
          "Yoti Session not found",
        );
        throw new AppError(
          HttpCodesEnum.SERVER_ERROR,
          "Yoti Session not found",
      ...
      if (completedYotiSessionInfo.state !== YotiSessionDocument.COMPLETED) {
        this.logger.error(
          { message: "Session in Yoti does not have status COMPLETED" },
          {
      ...
      if (idDocumentsDocumentFields.length === 0) {
        ...
        const documentTextDataCheck = completedYotiSessionInfo.checks.find(
          (check) => check.type === "ID_DOCUMENT_TEXT_DATA_CHECK",
        );
        ...
        this.logger.error(
          { message: "No document_fields found in completed Yoti Session" },
          {
            ...
            ID_DOCUMENT_TEXT_DATA_CHECK:
              documentTextDataCheck?.report?.recommendation,
          },
        );
        this.metrics.addMetric(
        ...
        await this.sendErrorMessageToIPVCore(
          f2fSession,
          "Yoti document_fields not populated",
        );
        throw new AppError(
          HttpCodesEnum.SERVER_ERROR,
          "Yoti document_fields not populated",
        ...
        await this.sendErrorMessageToIPVCore(
          f2fSession,
          "Multiple document_fields in response",
        );
        throw new AppError(
          HttpCodesEnum.SERVER_ERROR,
          "Multiple document_fields in response",
      ...
      if (!documentFieldsId) {
        this.logger.error(
          { message: "No media ID found in completed Yoti Session" },
          {
            messageCode: MessageCodes.VENDOR_SESSION_MISSING_DATA,
          },
        );
        ...
        await this.sendErrorMessageToIPVCore(
          f2fSession,
          "Yoti document_fields media ID not found",
        );
        throw new AppError(
          HttpCodesEnum.SERVER_ERROR,
          "Yoti document_fields media ID not found",
      ...
      const documentFields = await this.yotiService.getMediaContent(
        yotiSessionID,
        clientConfig.YotiBaseUrl,
        documentFieldsId,
      );
      ...
      if (!documentFields) {
        this.logger.error(
          { message: "No document fields info found" },
          {
            documentFieldsId,
            messageCode: MessageCodes.VENDOR_SESSION_MISSING_DATA,
          },
        ...
        await this.sendErrorMessageToIPVCore(
          f2fSession,
          "Yoti document fields info not found",
        );
        throw new AppError(
          HttpCodesEnum.SERVER_ERROR,
          "Yoti document fields info not found",
      ...
      if (
        ...
        const missingGivenName = this.checkMissingField(
          given_names,
          "given_names",
        );
        const missingFamilyName = this.checkMissingField(
          family_name,
          "family_name",
        );
        ...
        this.logger.info("Missing details check", {
          missingGivenName,
          missingFamilyName,
          missingFullName,
        });
        ...
        if (missingGivenName && missingFamilyName && missingFullName) {
          ...
          await this.sendErrorMessageToIPVCore(
            f2fSession,
            "Missing Name Info in DocumentFields",
          );
          throw new AppError(
            HttpCodesEnum.SERVER_ERROR,
            "Missing Name Info in DocumentFields",
        ...
        if (missingGivenName || missingFamilyName) {
          const personDetails = await this.f2fService.getPersonIdentityById(
            f2fSession.sessionId,
            this.environmentVariables.personIdentityTableName(),
          );
          if (!personDetails) {
            ...
            await this.sendErrorMessageToIPVCore(
              f2fSession,
              "Missing details in PERSON IDENTITY tables",
            );
            throw new AppError(
          ...
          VcNameParts = personIdentityUtils.getNamesFromPersonIdentity(
            personDetails,
            documentFields,
            this.logger,
          );
          ...
          VcNameParts = personIdentityUtils.getNamesFromYoti(
            given_names,
            family_name,
          );
        }

        const { credentialSubject, evidence, rejectionReasons } =
          this.generateVerifiableCredential.getVerifiedCredentialInformation(
            yotiSessionID,
            completedYotiSessionInfo,
            documentFields,
            VcNameParts,
          );
        ...
        if (!credentialSubject || !evidence) {
          ...
          await this.sendErrorMessageToIPVCore(
            f2fSession,
            "Missing Credential Subject or Evidence payload",
          );
          throw new AppError(
            HttpCodesEnum.SERVER_ERROR,
            "Missing Credential Subject or Evidence payload",
        ...
        try {
          unsignedJWT =
            this.verifiableCredentialService.generateVerifiableCredentialJwt(
              f2fSession,
              credentialSubject,
              evidence,
              absoluteTimeNow,
          ...
          if (unsignedJWT) {
            signedJWT =
              await this.verifiableCredentialService.signGeneratedVerifiableCredentialJwt(
                unsignedJWT,
              );
          }
        } catch (error) {
          if (error instanceof AppError) {
            this.logger.error(
              { message: "Error generating signed verifiable credential jwt" },
              {
                error,
            ...
            await this.sendErrorMessageToIPVCore(
              f2fSession,
              "Failed to sign the verifiableCredential Jwt",
            );
            return Response(
              HttpCodesEnum.SERVER_ERROR,
              "Failed to sign the verifiableCredential Jwt",
        ...
        try {
          ...
          this.logger.error(
            { message: "Failed to send VC to IPV Core Queue" },
            {
              error,
        ...
        await this.sendYotiEventsToTxMA(
          documentFields,
          VcNameParts,
          f2fSession,
          yotiSessionID,
        ...
        this.logger.error({
          message: "AuthSession is in wrong Auth state",
          sessionState: f2fSession.authSessionState,
        });
        this.metrics.addMetric(
        ...
        await this.sendErrorMessageToIPVCore(
          f2fSession,
          "AuthSession is in wrong Auth state",
        );
        return Response(
  ...
  checkMissingField(field: string, fieldName: string): boolean {
    if (!field || field.trim() === "") {
      this.logger.info({
        message: `Missing ${fieldName} field in documentFields response`,
      });
      return true;
    }
    return false;
  ...
  async sendYotiEventsToTxMA(
    ...
    let docName:
      | DocumentNames.PASSPORT
      | DocumentNames.DRIVING_LICENCE
      | DocumentNames.NATIONAL_ID;
    ...
    switch (documentFields.document_type) {
      ...
      case DocumentTypes.DRIVING_LICENCE:
        ...
        if (documentFields.issuing_country !== "GBR") {
          ...
          documentInfo.issueDate = documentFields.date_of_issue;
          documentInfo.fullAddress =
            documentFields.structured_postal_address?.formatted_address;
        }
        break;
      ...
      default:
        this.logger.error({
          message: `Unable to find document type ${documentFields.document_type}`,
  ...
  async sendErrorMessageToIPVCore(
    ...
    try {
      ...
      this.logger.error(
        { message: "Failed to send error message to IPV Core Queue" },
        {
          error,
          messageCode: MessageCodes.FAILED_SENDING_VC,
        },
