...
export class F2fService {
	...
	constructor(tableName: any, logger: Logger, dynamoDbClient: DynamoDBDocument, metrics: Metrics) {
	...
	static getInstance(tableName: string, logger: Logger, dynamoDbClient: DynamoDBDocument, metrics: Metrics): F2fService {
		if (!F2fService.instance) {
			F2fService.instance = new F2fService(tableName, logger, dynamoDbClient, metrics);
		}
	...
	async markSessionAsExpired(sessionId: string): Promise<void> {
		...
		try {
			...
			this.metrics.addMetric("F2F_SESSION_EXPIRED", MetricUnits.Count, 1);
		} catch (error) {
