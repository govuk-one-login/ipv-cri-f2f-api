...
//import {onstants} from "../utils/Contants";
...
export class GovNotifyService {
    ...
    async sendEmail(message: Email): Promise<EmailResponse> {
    	...
    	while (retryCount++ < this.environmentVariables.maxRetries() + 1) {
    		this.logger.debug(`sendEmail - trying to send email message ${GovNotifyService.name} ${new Date().toISOString()}`, { templateId: message.templateId, emailAddress: message.emailAddress, options });

    		try {
    				const response = await this.govNotify.sendEmail(message.templateId, message.emailAddress, options);
				     this.logger.debug("sendEmail - entire response after sending Email", response.data);
    				this.logger.debug("sendEmail - response after sending Email", GovNotifyService.name, response.status);
