...
export class SendToGovNotifyService {
    ...
    async sendYotiInstructions(sessionId: string): Promise<EmailResponse> {
        ...
        try {
            ...
            const f2fSessionInfo =
                await this.f2fService.getSessionById(sessionId);
            ...
            const f2fPersonInfo = await this.f2fService.getPersonIdentityById(
                sessionId,
            ...
            if (
                f2fPersonInfo.pdfPreference === PdfPreferenceEnum.PRINTED_LETTER
            ) {
                this.metrics.addMetric(
                    "SendToGovNotify_opted_for_printed_letter",
                    MetricUnit.Count,
                    1,
                );
                try {
                    const mergedPdf = await this.fetchPdfFile(
                        f2fSessionInfo,
                        this.environmentVariables.mergedLetterBucketPDFFolder(),
                    );
                    this.metrics.addMetric(
                        "SendToGovNotify_fetched_merged_pdf",
                        MetricUnit.Count,
                        1,
                    );
                    ...
                    if (mergedPdf) {
                        ...
                        await this.sendF2FLetterSentEvent(
                            f2fSessionInfo,
                            f2fPersonInfo,
                        );
                    }
                    ...
                    this.logger.error(
                        "sendYotiInstructions - Cannot send letter",
                        {
                            message: err,
                    ...
                    this.metrics.addMetric(
                        "SendToGovNotify_notify_letter_failed_generic_error",
                        MetricUnit.Count,
                        1,
                    );
                }
            ...
            const instructionsPdf = await this.fetchPdfFile(
                f2fSessionInfo,
                this.environmentVariables.yotiLetterBucketPDFFolder(),
            );

            if (instructionsPdf) {
                this.metrics.addMetric(
                    "SendToGovNotify_pdf_instructions_retreived",
                    MetricUnit.Count,
                    1,
                );
                ...
                const lastNameIndex =
                    f2fPersonInfo.name[0].nameParts.length - 1;

                const options = {
                    personalisation: {
                        [GOV_NOTIFY_OPTIONS.FIRST_NAME]:
                            f2fPersonInfo.name[0].nameParts[0].value,
                        [GOV_NOTIFY_OPTIONS.LAST_NAME]:
                ...
                await this.sendF2FYotiEmailedEvent(
                    f2fSessionInfo,
                    f2fPersonInfo,
                );
    ...
    async fetchPdfFile(
        ...
        this.logger.info("Fetching the pdf file from the S3 bucket. ", {
            bucket,
            key,
        });
        try {
            return await fetchEncodedFileFromS3Bucket(
                this.s3Client,
                bucket,
                key,
            );
            ...
            this.logger.error({
                message: "Error fetching the pdf file from S3 bucket",
                error,
                messageCode:
                    MessageCodes.ERROR_FETCHING_PDF_FILE_FROM_S3_BUCKET,
            });
            throw new AppError(
                HttpCodesEnum.SERVER_ERROR,
                "Error fetching the pdf file from S3 bucket",
            );
        }
    }

    async sendF2FYotiEmailedEvent(
        f2fSessionInfo: ISessionItem,
    ...
    async sendF2FLetterSentEvent(
        ...
        try {
            await this.f2fService.sendToTXMA({
                ...
                restricted: {
                    postalAddress: f2fPersonInfo.addresses.filter(
    ...
    async sendGovNotificationEmail(
        ...
        while (retryCount <= this.environmentVariables.maxRetries()) {
            ...
            try {
                ...
                this.metrics.addMetric(
                    "SendToGovNotify_email_sent_successfully",
                    MetricUnit.Count,
                    1,
                );
                ...
                singleMetric.addDimension(
                    "status_code",
                    emailResponse.status.toString(),
                );
                singleMetric.addMetric(
                    "SendToGovNotify_notify_email_response",
                    MetricUnit.Count,
                    1,
                );
                ...
                if (err.response) {
                    ...
                    singleMetric.addDimension(
                        "status_code",
                        err.response.data.status_code.toString(),
                    );
                    singleMetric.addMetric(
                        "SendToGovNotify_notify_email_response",
                        MetricUnit.Count,
                ...
                if (
                    ...
                    this.metrics.addMetric(
                        "SendToGovNotify_email_sent_failed_all_attempts",
                        MetricUnit.Count,
                        1,
                    );
                    throw appError;
    ...
    async sendGovNotificationLetter(
        ...
        while (retryCount <= this.environmentVariables.maxRetries()) {
            ...
            try {
                const letterResponse = await govNotify.sendPrecompiledLetter(
                    `${referenceId}-letter`,
                    pdf,
                );
                ...
                singleMetric.addDimension(
                    "status_code",
                    letterResponse.status.toString(),
                );
                singleMetric.addMetric(
                    "SendToGovNotify_notify_letter_response",
                    MetricUnit.Count,
                    1,
                );

                this.metrics.addMetric(
                    "SendToGovNotify_letter_sent_successfully",
                    MetricUnit.Count,
                    1,
                );
                ...
                if (err.response) {
                    this.logger.error(
                        `GOV UK Notify error ${SendToGovNotifyService.name}`,
                        {
                            statusCode: err.response.data.status_code,
                            errors: err.response.data.errors,
                        },
