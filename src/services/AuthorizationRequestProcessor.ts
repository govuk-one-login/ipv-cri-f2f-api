		...
		if (session != null) {
			...
			if (session.authSessionState === AuthSessionState.F2F_YOTI_SESSION_CREATED) {
				...
				try {
					...
					logger.error("Failed to write TXMA event F2F_CRI_AUTH_CODE_ISSUED to SQS queue.", {
				...
				try {
					...
					logger.error("Failed to write TXMA event F2F_CRI_END to SQS queue.", { error, messageCode: MessageCodes.FAILED_TO_WRITE_TXMA });
				}
