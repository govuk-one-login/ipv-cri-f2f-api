  ...
  async processRequest(): Promise<APIGatewayProxyResult> {
  	try {
  		...
  		if (F2FSessionCreatedRecords.length === 0) {
  			logger.info(`No users with session states ${[AuthSessionState.F2F_YOTI_SESSION_CREATED, AuthSessionState.F2F_AUTH_CODE_ISSUED, AuthSessionState.F2F_ACCESS_TOKEN_ISSUED]}`);
  		...
  		if (filteredSessions.length === 0) {
  			logger.info(`No users with session states ${[AuthSessionState.F2F_YOTI_SESSION_CREATED, AuthSessionState.F2F_AUTH_CODE_ISSUED, AuthSessionState.F2F_ACCESS_TOKEN_ISSUED]} older than 5 days`);
  			return { statusCode: HttpCodesEnum.OK, body: "No Sessions older than 5 days" };
  		}

  		logger.info("Total num. of users to send reminder emails to:", { numOfUsers: filteredSessions.length });
  			...
  			filteredSessions.map(async ({ sessionId, documentUsed }) => {
  				try {
  					...
  					if ( sessionItem && personIdentityItem ) {
  						...
  						logger.warn("No records returned from Person Identity or Session Table");
  						return null;
  					}
  				} catch (error) {
  					logger.error("Error fetching record from Person Identity Table", { error });
  				}
  			}),
  		);
  			...
  			.map(async ({ sessionId, yotiSessionId, emailAddress, firstName, lastName, documentUsed }) => {
  				try {
  					if (firstName && lastName && documentUsed) {
  						logger.info("Sending Dynamic Reminder Email", { sessionId });
  						...
  						logger.info("Sending Static Reminder Email", { sessionId });
  					...
  					logger.info("Reminder email sent to user: ", { sessionId });
  				} catch (error) {
  					logger.error("Failed to send reminder email or update flag", { error });
  				}
  			});
