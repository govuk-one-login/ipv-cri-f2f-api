...
export class ExpiredSessionsProcessor {
  ...
  constructor(
    ...
    const envVariables = new EnvironmentVariables(
      logger,
      ServicesEnum.REMINDER_SERVICE,
    );
    this.f2fService = F2fService.getInstance(
      envVariables.sessionTable(),
      logger,
      metrics,
  ...
  static getInstance(
    logger: Logger,
    metrics: Metrics,
  ): ExpiredSessionsProcessor {
    return (
      this.instance ||
      (this.instance = new ExpiredSessionsProcessor(logger, metrics))
    );
  }
  ...
  async processRequest(): Promise<APIGatewayProxyResult> {
    try {
      ...
      const records = await this.f2fService.getSessionsByAuthSessionStates(
        sessionStates,
        Constants.EXPIRED_SESSIONS_INDEX_NAME,
      );
      ...
      const yotiSessionTTL = process.env.YOTI_SESSION_TTL_DAYS
        ? +process.env.YOTI_SESSION_TTL_DAYS
        : NaN;
      if (isNaN(yotiSessionTTL)) {
        throw new Error(
          "YOTI_SESSION_TTL_DAYS is not defined or not a valid number.",
        );
      }

      const expirationTime =
        absoluteTimeNow() - (yotiSessionTTL + 1) * 24 * 60 * 60;
      ...
      if (!sessionsToExpire.length) {
        this.logger.info(
          `No users with session states ${sessionStates} older than ${yotiSessionTTL + 1} days`,
        );
        return {
          statusCode: HttpCodesEnum.OK,
      ...
      this.logger.info(
        ...
        { numOfExpiredSessions: sessionsToExpire.length },
      );
      ...
      await Promise.all(
        sessionsToExpire.map(async (session) => {
          try {
            ...
            this.logger.error(
              "Failed to send error message to IPV Core Queue",
              {
      ...
      this.logger.info("Successfully sent error message to IPV Core Queue", {
        count: ipvCoreSessionLogs.length,
        sessions: ipvCoreSessionLogs,
      });
      ...
      this.logger.info("Sessions marked as Expired", {
        count: markSessionsLogs.length,
        sessions: markSessionsLogs,
      });
